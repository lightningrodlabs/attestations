...
export default (orchestrator) => {
  ...
  orchestrator.registerScenario('test profile zomes', async (s, t) => {
    ...
    const [alice_attestations_happ, bob_attestations_happ] = await installAgents(conductor,  ["alice", "bobbo"])
    const [alice] = alice_attestations_happ.cells
    const [bobbo] = bob_attestations_happ.cells

    // Trigger init to run in both zomes
    await alice.call('attestations', 'list_channels', { category: "General" });
    await bobbo.call('attestations', 'list_channels', { category: "General" });
